...
val df3 = df2.groupBy("package_name","name").agg(sum("uptime"))
...
//sum uptime of the month
val totalUptime = df2.agg(sum("uptime")).first.getDouble(0) //todo: why must double?

//add pct column
val df5 = df4.withColumn("pct", col(df4.columns(2)).cast(LongType)/totalUptime)
